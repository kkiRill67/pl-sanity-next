...
// Variable: postQuery
// Query: *[_type == "post" && slug.current == $slug] [0] {    content[]{      _type,      _key,      stepNumber,      instruction,      image{        ...,        asset->{          _id,          _type,          url,          metadata{            dimensions{              width,              height            }          }        }      },      markDefs[]{        ...,          _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }      }    },      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  coverImage,  servings,  prepTime,  cookTime,  difficulty,  category,  protein,  fat,  carbs,  ingredients[]{    name,    amount,    notes  },  calories,  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
